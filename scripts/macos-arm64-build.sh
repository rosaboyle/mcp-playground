#!/bin/bash
# macOS arm64 (Apple Silicon) Build, Sign, Notarize and Publish Script
# This script handles the complete workflow for Apple Silicon Mac builds
...
echo -e "${GREEN}=== macOS arm64 (Apple Silicon) Build, Sign, Notarize and Publish ===${NC}"
...
echo -e "${GREEN}Building macOS arm64 (Apple Silicon) app with electron-builder...${NC}"
...
# Run electron-builder with arm64 architecture flag
DEBUG=electron-builder npx electron-builder build --mac --arm64 \
  --config.mac.notarize=false \
  --config.mac.hardenedRuntime=true \
  --config.mac.gatekeeperAssess=false \
  --config.afterSign="${AFTERSCRIPT_PATH}" \
  --config.artifactName='${productName}-${version}-arm64.${ext}'

# Find the .app file
APP_PATH=$(find "${RELEASE_DIR}/mac-arm64" -name "*.app" | head -n 1)
if [ -z "$APP_PATH" ]; then
  echo -e "${RED}Error: Cannot find .app file in ${RELEASE_DIR}/mac-arm64${NC}"
  exit 1
fi
...
# Find the DMG file
DMG_FILE=$(find "${RELEASE_DIR}" -name "*-arm64.dmg" | head -n 1)
if [ -z "$DMG_FILE" ]; then
  echo -e "${RED}Error: Cannot find -arm64.dmg file in ${RELEASE_DIR}${NC}"
  exit 1
fi
...
if [ "$SKIP_NOTARIZE" = false ]; then
  ...
  if [ -n "$SUBMISSION_ID" ]; then
    ...
    mkdir -p logs
    LOG_FILENAME="logs/notarization-arm64-${SUBMISSION_ID}-$(date +%Y%m%d-%H%M%S).log"
